# 环境安装指南

## 📋 目录

- [系统要求](#系统要求)
- [开发环境安装](#开发环境安装)
- [项目编译](#项目编译)
- [配置管理](#配置管理)
- [第一次运行](#第一次运行)
- [常见问题](#常见问题)

## 📦 系统要求

### 必需条件

| 组件 | 版本 | 说明 |
|------|------|------|
| **Java** | 21+ | 核心运行环境 |
| **Maven** | 3.8+ | 项目构建工具 |
| **Git** | 最新 | 版本控制 |

### 操作系统支持

- ✅ Windows 10/11
- ✅ macOS 10.15+
- ✅ Linux (Ubuntu 20.04+)

### 可选工具

- IDE: IntelliJ IDEA, Eclipse 或 VS Code
- 数据库: MySQL 8.0+ (可选，用于配置持久化)

## 🚀 开发环境安装

### Windows 系统

#### 1. 安装 Java 21

```bash
# 使用 Chocolatey 安装（推荐）
choco install openjdk21

# 验证安装
java -version

# 预期输出
# openjdk version "21.x.x" ...
```

#### 2. 安装 Maven

```bash
# 使用 Chocolatey
choco install maven

# 验证安装
mvn -version

# 预期输出
# Apache Maven 3.8.x ...
# Java version: 21.x.x ...
```

#### 3. 配置环境变量

```bash
# 设置 JAVA_HOME
setx JAVA_HOME "C:\Program Files\Java\jdk-21"

# 设置 MAVEN_HOME  
setx MAVEN_HOME "C:\Program Files\maven"

# 更新 PATH
setx PATH "%PATH%;%JAVA_HOME%\bin;%MAVEN_HOME%\bin"

# 重启命令行，验证
java -version
mvn -version
```

### macOS 系统

```bash
# 使用 Homebrew 安装
brew install openjdk@21
brew install maven

# 验证
java -version
mvn -version

# 配置环境变量（如需要）
# 编辑 ~/.zshrc 或 ~/.bash_profile
export JAVA_HOME=$(/usr/libexec/java_home -v 21)
export PATH="$JAVA_HOME/bin:$PATH"
```

### Linux 系统

```bash
# Ubuntu/Debian
sudo apt-get update
sudo apt-get install openjdk-21-jdk maven

# CentOS/RHEL
sudo yum install java-21-openjdk java-21-openjdk-devel maven

# 验证
java -version
mvn -version
```

## 🔧 项目编译

### 克隆项目

```bash
# 克隆主分支
git clone https://github.com/aibote/aibote4j.git
cd aibote4j

# 或克隆特定分支
git clone -b develop https://github.com/aibote/aibote4j.git
```

### 编译选项

```bash
# 1. 标准编译（推荐）
mvn clean compile

# 2. 快速编译（跳过测试）
mvn clean compile -DskipTests

# 3. 完整编译（包含测试）
mvn clean install

# 4. 生成可执行 JAR
mvn clean package

# 5. 跳过所有可选插件
mvn clean compile -o

# 6. 多线程编译（加快速度）
mvn clean compile -T 1C
```

### 验证编译结果

```bash
# 检查生成的 target 目录
ls target/

# 预期文件
# - classes/          # 编译后的类文件
# - generated-sources/ # 生成的代码
# - maven-status/     # 编译状态信息
```

## ⚙️ 配置管理

### 配置文件位置

```
sdk-common/src/main/resources/
├── bot-config.yml      # 机器人配置
└── logback.xml        # 日志配置（可选）
```

### 机器人配置（bot-config.yml）

```yaml
# 服务器配置
server:
  ip: "127.0.0.1"
  port: 9527
  timeout: 30000

# 通信配置
communication:
  responseTimeout: 10000      # 响应超时
  delayResponseTimeout: 30000 # 延迟响应超时
  poolSize: 10                # 连接池大小

# 日志配置
logging:
  level: INFO
  pattern: "%d{HH:mm:ss.SSS} [%thread] %-5level %logger{36} - %msg%n"

# 缓存配置
cache:
  enabled: true
  ttl: 3600           # 缓存过期时间（秒）
  maxSize: 1000       # 最大缓存数
```

### 运行时配置

```java
// 读取配置
ConfigManager configManager = ConfigManager.getInstance();
CommunicationConfig config = configManager.getCommunicationConfig();

// 获取配置值
int timeout = config.getResponseTimeout();
String serverIp = config.getServerIp();

// 动态设置（如需要）
config.setResponseTimeout(15000);
```

## 🎯 第一次运行

### 1. 验证环境

```bash
# 确认 Java 版本
java -version

# 确认 Maven 版本
mvn -version

# 检查 Git 安装
git --version
```

### 2. 编译项目

```bash
# 进入项目目录
cd aibote4j

# 清理并编译
mvn clean compile -DskipTests

# 预期输出
# [INFO] BUILD SUCCESS
```

### 3. 运行示例

```bash
# 运行 Windows 自动化示例
mvn exec:java -Dexec.mainClass="net.aibote.examples.WinBotExample"

# 或编译后运行
cd sdk-server
mvn spring-boot:run
```

### 4. 检查连接

```java
// 创建简单测试
WinBot bot = BotFactory.builder()
    .withBotType(BotFactory.BotType.WIN)
    .build();

// 测试连接
if (bot.connect()) {
    System.out.println("连接成功！");
    bot.disconnect();
} else {
    System.out.println("连接失败");
}
```

## 🐛 常见问题

### Q1: Maven 编译失败，错误为 "Unknown class file version"

**原因**：Java 版本过低

**解决**：
```bash
# 检查 Java 版本
java -version

# 应该显示 21.x.x 或更高
# 如果版本低于 21，升级 Java
```

### Q2: 编译时报 "No JDK found"

**原因**：Maven 找不到 JDK

**解决**：
```bash
# 设置 JAVA_HOME
export JAVA_HOME=/path/to/jdk-21  # Linux/macOS
setx JAVA_HOME "C:\Program Files\Java\jdk-21"  # Windows

# 验证
echo $JAVA_HOME

# 重新编译
mvn clean compile
```

### Q3: 网络连接失败

**原因**：连接参数错误或目标不可达

**解决**：
```bash
# 检查配置
cat sdk-common/src/main/resources/bot-config.yml

# 测试网络连接
ping 127.0.0.1

# 检查防火墙
# 确保端口 9527 未被阻止
```

### Q4: 内存不足 (OutOfMemoryError)

**原因**：JVM 内存分配不足

**解决**：
```bash
# 增加 Maven 内存
export MAVEN_OPTS="-Xmx1024m -Xms512m"

# 或在 pom.xml 中配置
<maven.compiler.source>21</maven.compiler.source>
<maven.compiler.target>21</maven.compiler.target>
```

### Q5: 编译成功但无法运行

**原因**：依赖未下载或路径错误

**解决**：
```bash
# 清理缓存并重新下载依赖
mvn clean dependency:resolve

# 检查依赖
mvn dependency:tree

# 重新编译
mvn clean compile
```

## 📚 进阶配置

### 自定义日志配置

创建 `logback.xml`：

```xml
<?xml version="1.0" encoding="UTF-8"?>
<configuration>
    <appender name="CONSOLE" class="ch.qos.logback.core.ConsoleAppender">
        <encoder>
            <pattern>%d{HH:mm:ss.SSS} [%thread] %-5level %logger{36} - %msg%n</pattern>
        </encoder>
    </appender>
    
    <root level="INFO">
        <appender-ref ref="CONSOLE"/>
    </root>
</configuration>
```

### 远程连接配置

```java
// 连接到远程 Windows Driver
WinBot bot = BotFactory.builder()
    .withBotType(BotFactory.BotType.WIN)
    .withIP("192.168.1.100")        // 远程 IP
    .withPort(9527)                 // 远程端口
    .withTimeout(30000)             // 超时时间
    .build();

bot.connect();
```

### 连接池配置

```java
// 创建连接池管理器
ConnectionPoolManager poolManager = ConnectionPoolManager.getInstance();

// 配置池参数
poolManager.setPoolSize(20);
poolManager.setConnectionTimeout(5000);
poolManager.setIdleTimeout(300000);

// 监控池状态
System.out.println("活跃连接: " + poolManager.getActiveConnections());
System.out.println("空闲连接: " + poolManager.getIdleConnections());
```

## ✅ 安装检查清单

- [ ] Java 21+ 已安装
- [ ] Maven 3.8+ 已安装
- [ ] 项目已克隆
- [ ] 项目编译成功
- [ ] 环境变量已配置
- [ ] 可以运行示例代码
- [ ] 可以连接到设备
- [ ] 日志输出正常

## 📖 下一步

✅ 安装完成后，请阅读 [快速开始](03-快速开始.md) 文档。