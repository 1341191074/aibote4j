...
本章详细介绍如何诊断和解决 Aibote4J 框架中的各类错误。
...
try {
    ...
    log.info("Attempting to connect to {}:{}", "127.0.0.1", 9527);
    
    boolean connected = bot.connect();
    if (!connected) {
        log.error("Connection failed to {}:{}", "127.0.0.1", 9527);
        
        // 检查网络连通性
        try {
            InetAddress addr = InetAddress.getByName("127.0.0.1");
            boolean reachable = addr.isReachable(5000); // 5秒超时
...
// 尝试增加超时时间
configManager.getCommunicationConfig().setResponseTimeout(30000); // 30秒
...
public void diagnoseCommandExecution(AbstractPlatformBot bot) {
    ...
    try {
        ...
        // 执行诊断命令
        String result = bot.getWindowSize(); // 或其他简单的命令
...
public void diagnoseOCR(AndroidBot bot) {
    ...
    try {
        ...
        // 检查 OCR 服务是否初始化
        // 可以尝试执行一个简单的 OCR 操作
        Region region = new Region(0, 0, 100, 100); // 小区域测试
