# 常见问题

本章收集了使用 Aibote4J 框架时最常见的问题及其解决方案。

## 📦 编译问题

### 1. "Unknown class file version" 错误

**症状：**
```
error: Source option 1.8 is no longer supported. Use 21 or later.
```

**原因：** Java 版本不匹配

**解决方案：**
```bash
# 检查当前 Java 版本
java -version

# 应该输出 Java 21 或更高
# 如果版本过低，升级 JDK

# 验证 Maven 使用的 Java 版本
mvn -version

# 清理缓存重新编译
mvn clean compile
```

### 2. "No JDK found" 错误

**症状：**
```
[ERROR] No compiler is provided in this environment
```

**原因：** Maven 使用了 JRE 而不是 JDK

**解决方案：**
```bash
# 设置 JAVA_HOME 指向 JDK

# Windows
setx JAVA_HOME "C:\Program Files\Java\jdk-21"

# Linux/macOS
export JAVA_HOME=/usr/libexec/java_home -v 21
# 或编辑 ~/.bashrc 或 ~/.zshrc

# 验证设置
echo $JAVA_HOME
java -version

# 重新编译
mvn clean compile
```

### 3. "Maven 找不到依赖" 错误

**症状：**
```
[ERROR] Failed to execute goal on project xxx: Could not resolve dependencies
```

**原因：** 网络问题或 Maven 仓库配置错误

**解决方案：**
```bash
# 检查网络连接
ping repo.maven.apache.org

# 清理本地缓存
mvn clean dependency:purge-local-repository

# 重新下载依赖
mvn dependency:resolve

# 如果仍失败，检查 Maven 配置
cat ~/.m2/settings.xml  # Linux/macOS
type %USERPROFILE%\.m2\settings.xml  # Windows

# 使用国内镜像（如需要）
# 编辑 ~/.m2/settings.xml，添加：
# <mirror>
#   <id>aliyun</id>
#   <mirrorOf>central</mirrorOf>
#   <url>http://maven.aliyun.com/nexus/content/groups/public/</url>
# </mirror>

# 重新编译
mvn clean compile
```

### 4. "内存不足" 错误

**症状：**
```
java.lang.OutOfMemoryError: Java heap space
```

**原因：** JVM 堆内存分配不足

**解决方案：**
```bash
# 增加 Maven JVM 内存
export MAVEN_OPTS="-Xmx2048m -Xms1024m"

# 或临时设置（单次编译）
mvn clean compile -DargLine="-Xmx2048m"

# 重新编译
mvn clean compile
```

## 🔌 连接问题

### 1. "连接被拒绝" 错误

**症状：**
```
Connection refused: connect
或
java.net.ConnectException: Connection refused
```

**原因：** 目标服务未启动或地址/端口错误

**解决方案：**
```bash
# 检查服务是否运行
netstat -an | findstr 9527  # Windows
netstat -an | grep 9527      # Linux/macOS

# 如果未运行，启动服务
cd sdk-server
mvn spring-boot:run

# 验证连接
# 1. 检查 IP 配置
# 2. 检查端口配置
# 3. 尝试 ping
ping 127.0.0.1

# 检查防火墙
# Windows：检查 Windows Defender 防火墙
# Linux：检查 iptables 或 ufw
```

### 2. "连接超时" 错误

**症状：**
```
java.net.SocketTimeoutException: Connection timed out
或
Read timed out
```

**原因：** 网络延迟大或服务响应慢

**解决方案：**
```java
// 增加超时时间
WinBot bot = BotFactory.builder()
    .withBotType(BotFactory.BotType.WIN)
    .withTimeout(60000)  // 60 秒超时
    .build();

// 或在配置文件中修改
// bot-config.yml:
// communication:
//   responseTimeout: 60000
```

```bash
# 检查网络连接
ping -c 10 127.0.0.1  # Linux/macOS
ping -n 10 127.0.0.1  # Windows

# 检查网络延迟
tracert 127.0.0.1  # Windows
traceroute 127.0.0.1  # Linux
```

### 3. "远程地址无效" 错误

**症状：**
```
java.net.UnknownHostException: hostname
或
getaddrinfo failed
```

**原因：** 主机名无法解析或 IP 地址错误

**解决方案：**
```bash
# 检查主机名解析
nslookup hostname         # Windows
dig hostname             # Linux/macOS
ping hostname

# 检查 IP 地址
# 使用 ipconfig (Windows) 或 ifconfig (Linux)

# 确保 IP 和端口正确
# bot-config.yml:
# server:
#   ip: "192.168.1.100"  # 正确的 IP
#   port: 9527           # 正确的端口
```

## ⚡ 运行时错误

### 1. "NullPointerException" 错误

**症状：**
```
java.lang.NullPointerException: Cannot invoke method on null object
```

**原因：** 未初始化或连接失败

**解决方案：**
```java
// 确保连接成功
WinBot bot = BotFactory.builder()
    .withBotType(BotFactory.BotType.WIN)
    .build();

// 检查任务注册状态
String taskId = TaskEngine.getInstance().registerTask("debug-task", task);
if (taskId == null) {
    throw new RuntimeException("任务注册失败");
}

// 使用前检查 bot 非空
if (bot == null) {
    System.err.println("Bot 未初始化");
    return;
}

// 操作前检查返回值
String result = bot.getWindowSize();
if (result == null || result.isEmpty()) {
    System.err.println("获取窗口大小失败");
    return;
}
```

### 2. "类未找到" 错误

**症状：**
```
java.lang.ClassNotFoundException: net.aibote.sdk.WinBot
或
NoClassDefFoundError
```

**原因：** 依赖未正确加载或 classpath 配置错误

**解决方案：**
```bash
# 检查依赖
mvn dependency:tree

# 清理并重新编译
mvn clean compile -U

# 确保 JAR 在 classpath 中
# 检查 target/classes 目录

# 如果使用 IDE，重新构建项目
# IntelliJ: Build > Rebuild Project
# Eclipse: Project > Clean
# VS Code: 重新加载窗口
```

### 3. "参数验证失败" 错误

**症状：**
```
java.lang.IllegalArgumentException: Invalid parameter
或
Parameter validation failed
```

**原因：** 传入的参数无效

**解决方案：**
```java
// 验证参数
String xpath = "/hierarchy/android.widget.Button";
if (xpath == null || xpath.isEmpty()) {
    throw new IllegalArgumentException("XPath 不能为空");
}

// 检查坐标范围
int x = 500, y = 800;
if (x < 0 || y < 0) {
    throw new IllegalArgumentException("坐标不能为负数");
}

// 检查文件路径
String imagePath = "image.png";
File file = new File(imagePath);
if (!file.exists()) {
    throw new FileNotFoundException("图片文件不存在: " + imagePath);
}
```

## 🚀 性能问题

### 1. "内存泄漏" 症状

**表现：** 程序运行一段时间后内存持续增长

**原因：** 未及时释放资源或连接

**解决方案：**
```java
// 确保使用 try-finally 或 try-with-resources
try {
    WinBot bot = BotFactory.builder()
        .withBotType(BotFactory.BotType.WIN)
        .build();
    
    // 通过任务执行
    TaskEngine.getInstance().executeTask("debug-task", channelContext, BotFactory.BotType.WIN);
} finally {
    // 确保断开连接
    if (bot != null) {
        bot.disconnect();
    }
}

// 任务驱动的正确方式
NotepadAutomationTask task = NotepadAutomationTask.builder()
    .taskName("调试任务")
    .scriptName("Debug-Task")
    .build();

TaskEngine.getInstance().registerTask("debug-task", task);
Application.main(new String[]{});
```

### 2. "响应缓慢" 问题

**表现：** 命令执行缓慢，超时频繁

**原因：** 网络延迟、服务器负载高、或算法效率低

**解决方案：**
```bash
# 检查网络延迟
ping -c 100 127.0.0.1  # Linux/macOS

# 检查系统资源
top  # Linux
taskmgr  # Windows
Activity Monitor  # macOS

# 增加超时时间
export MAVEN_OPTS="-Xmx2048m"

# 使用连接池（如支持）
ConnectionPoolManager.getInstance().setPoolSize(20);
```

### 3. "CPU 占用率高" 问题

**表现：** 某个进程占用 CPU 过高

**原因：** 死循环、轮询过频、或算法不优化

**解决方案：**
```java
// 避免死循环
while (true) {
    // ❌ 不要这样做
    // bot.click(x, y);
}

// 添加延迟
Thread.sleep(100);  // 每次操作间隔 100ms

// 或使用计数器
int maxRetries = 10;
for (int i = 0; i < maxRetries; i++) {
    if (bot.findImage("target.png") != null) {
        break;
    }
    Thread.sleep(500);
}
```

## ❓ 一般问题

### Q1: 如何调试应用？

**答：** 使用 IDE 调试功能或添加日志

```java
// 方法 1：使用 IDE 调试
// IntelliJ: Run > Debug
// Eclipse: Debug As > Java Application

// 方法 2：添加日志
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

Logger logger = LoggerFactory.getLogger(MyClass.class);
logger.debug("调试信息: {}", variable);
logger.error("错误信息", exception);
```

### Q2: 如何处理多个设备？

**答：** 创建多个 bot 实例并管理连接

```java
// 注册多个任务
NotepadAutomationTask task1 = NotepadAutomationTask.builder()
    .taskName("设备1任务")
    .scriptName("Device1-Task")
    .build();

NotepadAutomationTask task2 = NotepadAutomationTask.builder()
    .taskName("设备2任务")
    .scriptName("Device2-Task")
    .build();

TaskEngine.getInstance().registerTask("device1-task", task1);
TaskEngine.getInstance().registerTask("device2-task", task2);

// 启动服务端，等待多个客户端连接
Application.main(new String[]{});
```

### Q3: 如何提高图像识别准确率？

**答：** 调整参数和图片质量

```java
// 调整相似度阈值
Point point = bot.findImage("target.png", 0.95f);  // 95% 相似度

// 使用更好质量的图片
bot.takeScreenshot("screenshot.png");

// 指定搜索区域
Region region = new Region(100, 100, 500, 500);
point = bot.findImage("target.png", 0.9f, region);
```

### Q4: 如何处理随机出现的错误？

**答：** 实现重试机制

```java
// 重试机制
int maxAttempts = 3;
int attempt = 0;
boolean success = false;

while (attempt < maxAttempts && !success) {
    try {
        bot.click(500, 600);
        success = true;
    } catch (Exception e) {
        attempt++;
        if (attempt >= maxAttempts) {
            throw e;
        }
        Thread.sleep(1000 * attempt);  // 指数退避
    }
}
```

### Q5: 如何优化 OCR 识别性能？

**答：** 调整识别参数和区域

```java
// 限制 OCR 识别区域
Region region = new Region(100, 100, 800, 600);
List<OCRResult> results = bot.ocrByHwnd(hwnd, region, 0, 0, 0, Mode.FOREGROUND);

// 调整二值化参数
List<OCRResult> results = bot.ocrByHwnd(hwnd, region, 1, 127, 255, Mode.FOREGROUND);
```